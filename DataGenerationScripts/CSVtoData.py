...
HORSE_KEYPOINT_ORDER = [
    ...
    'nose', 'left_eye', 'right_eye', 'left_earbase', 'rightt_earbase', 'neck_base', 
    'neck_end', 'throat_end', 'back_base', 'back_end'
]
...
#     'front_left_thai', 'front_right_thai', 'back_left_thai', 'back_right_thai', 
#     'nose', 'left_eye', 'right_eye', 'left_earbase', 'rightt_earbase', 'neck_base', 
#     'neck_end', 'throat_end', 'back_end', 'back_middle', 'belly_bottom', 'body_middle_left',
#     'body_middle_right'
# ]
...
def createIMGsAndTxt(video_path, human_pose_csv, horse_pose_csv_path,
                     ...
                     validimages_dir, validlabels_dir, trainimages_dir_no_folder, 
                     trainlabels_dir_no_folder, validimages_dir_no_folder, 
                     validlabels_dir_no_folder):
    ...
    train_img_count = len(list(Path(trainimages_dir_no_folder).glob("*.jpg")))
    valid_img_count = len(list(Path(validimages_dir_no_folder).glob("*.jpg")))
    ...
    while cap.isOpened():
        ...
        if best_frame_data and best_score >= MINIMUM_QUALITY_THRESHOLD:
            ...
            for kpt_name in HUMAN_KEYPOINT_ORDER:
                ...
                # ## FIX: A keypoint at (0,0) cannot be visible.
                is_visible = kpt_s >= CONFIDENCE_THRESHOLD and (kpt_x != 0 or kpt_y != 0)
                vis = VISIBILITY_LABELED_VISIBLE if is_visible else VISIBILITY_NOT_LABELED
                all_keypoints_str_parts.extend([f"{kpt_x / img_width:.{NORMALIZED_DECIMAL_PLACES}f}", f"{kpt_y / img_height:.{NORMALIZED_DECIMAL_PLACES}f}", f"{vis:.1f}"])
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        base_dir = Path(".") 
        csv_base_dir = base_dir / "CSVOutputsValid"
        dataset_base_dir = base_dir / "ultimateDataset"
        ...
        createIMGsAndTxt(
            ...
            str(valid_dir_images), str(valid_dir_labels), str(train_dir_images_no_folder),
            str(train_dir_labels_no_folder), str(valid_dir_images_no_folder),
            str(valid_dir_labels_no_folder)
        )
    else:
